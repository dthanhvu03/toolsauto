...
class AIStudioService:
    @staticmethod
    def get_studio_context(db: Session):
        base_personas = {
        ...
        platforms = {
            "threads": {
                ...
                "label": "Threads — AI prompt",
                "icon": '<svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"></path></svg>',
            },
        }
        overrides = runtime_settings.get_overrides(db, use_cache=False)
        for info in list(base_personas.values()) + list(configs.values()) + list(platforms.values()):
        ...
        return {"personas": base_personas, "configs": configs, "platforms": platforms}

    @staticmethod
    def save_setting(db: Session, key: str, value: str, updated_by: str = None):
        if key not in runtime_settings.AI_STUDIO_TEMPLATE_KEYS:
            raise ValueError(f"Key không thuộc AI Studio: {key}")
    ...
    async def run_test(prompt_content: str, niche: str) -> Tuple[Optional[Dict[str, Any]], Optional[str], Optional[list]]:
        try:
            ...
            result, meta = await AIUseCases.generate_text_async(
                mega_prompt, purpose=AIPurpose.STUDIO_TEST
            )
            if not result:
