...
## 🏛️ 4. Quy tắc Kiến trúc Đa tầng (Layered Architecture Discipline)
Để tránh nợ kỹ thuật và giữ codebase chuẩn Enterprise:
-   **Router (Controller)**: Tầng này chỉ được làm 3 việc: Nhận Request $\rightarrow$ Validate $\rightarrow$ Gọi Service. Tuyệt đối KHÔNG viết câu lệnh SQL thô (`db.execute`), KHÔNG gọi bash (`subprocess`), KHÔNG chứa logic nghiệp vụ.
-   **Schema (DTO/Validation)**: Mọi Pydantic Models phải được gom về chung thư mục `app/schemas/`. Tuyệt đối KHÔNG viết chui class BaseModel bên trong file của Router.
-   **Service (Business Logic)**: Tránh xa mô hình "God Service" (ôm đồm quá nhiều việc). Các hệ thống cốt lõi (như AI) phải là Nguồn Chân Lý Duy Nhất (Single Source of Truth), cấm nhân bản nhiều đường ống.
-   **Adapter (External API)**: Tầng điều phối (Dispatcher) phải "mù" nền tảng, cấm hardcode rẽ nhánh kiểu `if platform == "facebook"`. Mọi Logic bên ngoài phải bọc Python Decorators bắt lỗi thống nhất để đảm bảo DRY (Don't Repeat Yourself).
-   **Worker (Background Job)**: Hạn chế thiết kế bot spam query Database bằng `while True: sleep()` (Database Polling). Ưu tiên Event-driven.
