...
SETTINGS: dict[str, SettingSpec] = {
    "DRIVE_COPY_ENABLED": SettingSpec(
        ...
        title="Bật sao lưu ngoại vi",
        section="Sao lưu Google Drive",
        description=(
            "Chép thêm một bản sang thư mục Google Drive đã gắn. Backup hiện nằm cùng ổ đĩa "
            "với dữ liệu, hỏng ổ là mất cả hai. Drive chưa gắn ổ thì chỉ ghi cảnh báo, "
            "không làm hỏng backup local."
        ),
    ),
    "DRIVE_ROOT_DIR": SettingSpec(
        ...
        section="Sao lưu Google Drive",
        description=(
            "Đường dẫn thư mục Google Drive for Desktop, ví dụ G:/My Drive/ToolsAuto. "
            "Kiểm tra trước bằng: python manage.py drive check"
        ),
    ),
    "DRIVE_COPY_BACKUPS": SettingSpec(
        ...
        title="Chép bản dump database",
        section="Sao lưu Google Drive",
        description="Mỗi lần backup xong thì chép thêm một bản vào Drive/backups.",
    ),
    ...
    "monitor.hc_url_backup": SettingSpec(
        ...
        title="Ping: backup database",
        section="Giám sát",
        description="URL ping healthchecks.io (hc-ping.com/<uuid>). Để trống = tắt.",
    ),
    ...
    "monitor.hc_url_publisher": SettingSpec(
        ...
        type="str",
        default_getter=lambda: "",
        title="Ping: worker đăng Facebook",
        section="Giám sát",
        description="URL ping healthchecks.io (hc-ping.com/<uuid>). Để trống = tắt.",
    ),
    ...
    "ai.whisper_model_size": SettingSpec(
        ...
        choices=[
            ...
            "large-v3-turbo", "erax-ai/EraX-WoW-Turbo-V1.1-CT2",
        ],
        enum_labels={
            "tiny": "Tiny — nhanh, nhẹ",
            "base": "Base — cân bằng",
            ...
            "large-v3-turbo": "Large-v3-turbo — chinh xac nhat, nang RAM (~1.6GB)",
            "erax-ai/EraX-WoW-Turbo-V1.1-CT2": "EraX-WoW-Turbo — fine-tune tieng Viet, MIT",
        },
    ),
    # ADR-016: lop them cho video reup (phu de dot). Mac dinh TAT — tat het => reup y nhu cu.
    "reup.subtitle_enabled": SettingSpec(
        key="reup.subtitle_enabled",
        type="bool",
        default_getter=lambda: False,
        title="Đốt phụ đề tiếng Việt",
        section="Reup — lớp thêm",
        description="Đốt phụ đề tiếng Việt (Whisper) vào video reup. Mặc định tắt.",
    ),
    "reup.subtitle_font_size": SettingSpec(
        key="reup.subtitle_font_size",
        type="int",
        default_getter=lambda: 64,
        title="Cỡ chữ phụ đề",
        section="Reup — lớp thêm",
        description="Cỡ chữ tính trên khung 1080×1920 (libass tự scale theo video).",
        min=24,
        max=120,
        unit="px",
    ),
    "reup.subtitle_margin_v": SettingSpec(
        key="reup.subtitle_margin_v",
        type="int",
        default_getter=lambda: 260,
        title="Khoảng cách đáy phụ đề",
        section="Reup — lớp thêm",
        description="Lề dưới (MarginV) trên khung 1080×1920 để phụ đề không đè lên thanh caption TikTok/Reels.",
        min=40,
        max=800,
        unit="px",
    ),
    "reup.audio_voice_enabled": SettingSpec(
        key="reup.audio_voice_enabled",
        type="bool",
        default_getter=lambda: False,
        title="Voice-over đọc hook text",
        section="Reup — lớp thêm",
        description="Đọc hook text bằng TTS tiếng Việt (edge-tts, dự phòng piper offline) trộn vào video. Mặc định tắt.",
    ),
    "reup.audio_voice_name": SettingSpec(
        key="reup.audio_voice_name",
        type="enum",
        default_getter=lambda: "vi-VN-HoaiMyNeural",
        title="Giọng đọc",
        section="Reup — lớp thêm",
        description="Giọng edge-tts. HoaiMy = nữ, NamMinh = nam.",
        choices=["vi-VN-HoaiMyNeural", "vi-VN-NamMinhNeural"],
    ),
    "reup.audio_voice_engine": SettingSpec(
        key="reup.audio_voice_engine",
        type="enum",
        default_getter=lambda: "auto",
        title="Engine TTS",
        section="Reup — lớp thêm",
        description="auto = edge-tts trước, hỏng thì piper offline (model trong storage/media/tts_models/).",
        choices=["auto", "edge", "piper"],
    ),
    "reup.audio_voice_rate": SettingSpec(
        key="reup.audio_voice_rate",
        type="str",
        default_getter=lambda: "+0%",
        title="Tốc độ đọc",
        section="Reup — lớp thêm",
        description="Ví dụ -20% … +50%.",
    ),
    "reup.audio_original_volume": SettingSpec(
        key="reup.audio_original_volume",
        type="str",
        default_getter=lambda: "0.25",
        title="Âm lượng tiếng gốc khi có voice-over",
        section="Reup — lớp thêm",
        description="0.0 – 1.0",
    ),
    "reup.audio_music_enabled": SettingSpec(
        key="reup.audio_music_enabled",
        type="bool",
        default_getter=lambda: False,
        title="Nhạc nền",
        section="Reup — lớp thêm",
        description="Chọn ngẫu nhiên 1 file trong storage/media/music/ (tự tải — Meta Sound Collection chỉ dùng trên Facebook). Mặc định tắt.",
    ),
    "reup.audio_music_volume": SettingSpec(
        key="reup.audio_music_volume",
        type="str",
        default_getter=lambda: "0.12",
