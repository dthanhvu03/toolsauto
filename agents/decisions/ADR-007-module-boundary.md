...
> **Note**: `app/core/compliance/` BỊ LOẠI khỏi core. Xem rationale bên dưới.
...
| `app/features/facebook_publisher/` | adapter + media_processor + compliance (fb_compliance + checker) + reup_pipeline + router | publisher (FB) |
| `app/features/instagram/` | adapter + router (minimal) | — |
| `app/features/tiktok/` | adapter (read-only viral discovery) | — |
| `app/features/viral_intake/` | orchestrator + discovery + scan + tiktok_scraper + video_protector + reup_processor + strategic | ai_generator |
...
### Tại sao compliance KHÔNG vào core?

1. Class chính gọi `FBComplianceChecker` — hard-coded cho Facebook.
2. `check_before_publish()` raise `CompliancePublishError` với message "vi phạm chính sách Facebook".
3. `rewrite()` prompt: "chuyên gia nội dung Facebook".
4. Không có Threads/IG compliance checker.
5. YAGNI: tách generic compliance engine khi chưa có consumer thứ 2 là premature abstraction.
6. Nếu sau này cần compliance cho Threads → extract interface lên `core/compliance/`, implement per-feature. Lúc đó refactor 1 file, không break gì.
